-- 0015_register_pages (postgres): the indexes the two supply-chain registers' keyset walks and
...
-- is one row-value comparison (`(last_seen_at, format, name, version) < (…)`), which Postgres
-- plans as a single index seek. A btree serves an ORDER BY only when its direction pattern
-- matches the statement's or is its exact mirror; a mixed `last_seen_at DESC, name ASC` would
-- match neither of these and would sort. The plan tests in `db-tests/tests/postgres.rs` assert
-- the seek for the statement the repository emits — they read the SQL from the repository
-- rather than copying it (roadmap D52).
--
-- **Privileges:** nothing to grant. The S-22 app-role template in `0002_identity.sql` grants at
-- table level and narrows only `audit_log`; indexes carry no privileges of their own, and the
-- two new delete statements are on tables the app role may already delete from.
...
-- acknowledged_at < $1`. Partial on the same predicate the statement carries, so an instance
