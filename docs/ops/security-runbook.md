...
Honestly: **KEK rotation is structurally impossible in the current build** (roadmap D27). There is one `auth.kek`, no previous-KEK slot, and no DEK layer between it and the data — S-27's "KEK rotation = DEK rewrap" describes the *intended* design, not the implemented one. The KEK seals every TOTP seed ([S-05](../security.md#1-authentication)), the SMTP password stored via runtime settings, and the body of every queued message ([S-26](../security.md#6-secrets--configuration)) — a queued body the KEK cannot open dead-letters on its **first** attempt rather than retrying, so a KEK change silently discards in-flight mail.
...
- **Nobody can sign in, and outbound mail is the reason** — the one break-glass case that is *not* an API call, because on an OTP-only instance the session every other lever needs is what mail delivers. Symptoms: `mail_transport_unusable == 1` (see [monitoring.md](monitoring.md)), a `mail.delivery_failed` audit row, `drain (N dead)` in the admin jobs table, or simply "the code never arrives". Note that **correcting `[smtp]` in the config file and restarting does not help**: a stored `smtp` section replaces the boot one wholesale, so a bad section written through the admin UI outlives every restart. The recovery, with the container stopped or on any host that can reach the database:

  ```sh
  pubd reset-smtp --dry-run   # prints what it would delete and what the boot config would become
  pubd reset-smtp             # deletes the stored smtp section
  ```

  The boot `[smtp]` becomes authoritative again; a *running* instance picks the change up through the settings version poll within one interval, with no restart. Fix `[smtp]` in the config file first if it is empty or wrong — `reset-smtp` restores the boot section, it does not invent one. Queued mail retries on the next drain tick; anything already dead-lettered is not requeued (there is no requeue endpoint), so have affected users request a new code. `reset-smtp` reads your normal configuration (so it needs one that loads and validates — the instance it rescues has one), touches only the settings table, and starts no listener.
