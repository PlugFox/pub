...
What exists today, per plane — S-27's single "revoke all tokens/sessions for user/org" admin action is not one endpoint yet (roadmap [D37](../roadmap.md#medium)), and the pieces that do exist sit on **two separate authority ladders**: the instance admin on one, org roles on the other ([decision 19](../decisions.md#19--rbac-cumulative-role-levels-with-a-single-authorize-chokepoint) — an org Owner administers their org, not the instance, and the reverse holds too). Neither ladder alone neutralizes a compromised account, and one case has no complete answer at all — read this section before you need it.

- **A user's own credentials**: `POST /api/v1/sessions/revoke-all` (step-up-gated) kills every session; `DELETE /api/v1/tokens/{id}` revokes a CLI token, effective everywhere within ≤ 60 s ([S-13](../security.md#3-cliapi-tokens)). Only the token's owner can revoke the token row — there is no admin path to it.
- **Instance admin, against a compromised account**: `POST /api/v1/admin/users/{id}/suspend` blocks sign-in and revokes **every session** immediately ([S-09](../security.md#2-sessions--web-plane)). That is the whole instance-admin arsenal. Suspension does **not** revoke or disable the account's CLI tokens — token authentication checks the token row and re-derives authority from org membership, never from the user's status — and the instance-admin role carries **no org-management power**: it cannot remove the account's memberships. That lever belongs to Admin+ *inside* each org.
- **In-org Admin+, same incident**: removing the compromised account from the org (`DELETE /api/v1/orgs/{slug}/members/{user_id}`) leaves its tokens authenticating but authorizing nothing, effective on the next request, because token authority is re-derived per request from the durable membership. The removal also revokes the account's sessions ([S-09.a](../security.md#2-sessions--web-plane)) — as does any role change; that part is self-enforcing. Full neutralization is therefore **suspension plus membership removal — two actors on two ladders**, unless one person happens to wear both hats.
...
- **The sole-Owner case, honestly**: if the compromised account is an org's **only Owner**, membership removal and demotion both fail with `409 last_owner`, and no other member outranks it. Suspension still cuts the web plane, but the account's org-bound tokens stay live *and authorized*. If the org has another Owner, that Owner can remove the compromised account normally — or archive the org outright (previous bullet), which does revoke the tokens. With no other Owner, **no action available today disables those tokens**: the exposure stands until they expire or until the admin token-revocation endpoint exists (roadmap [D37](../roadmap.md#medium)). Mint Owner tokens with short `expires_days`, and prefer two Owners per org, precisely for this page.
