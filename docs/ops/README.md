...
| [reverse-proxy.md](reverse-proxy.md) | nginx / Caddy / Traefik examples: TLS, `public_url` correctness, `trust_proxy_headers`, SSE, upload sizes and timeouts, and the multi-upstream form |
...
| [capacity.md](capacity.md) | What one measurement actually said: read and publish latency against the two-replica stand, where the rate budgets bite, and what has never been measured |
...
a **rolling upgrade** of a multi-replica deployment is neither documented nor tested — the two-replica stand ([install.md](install.md#the-two-replica-stand)) proves four claims about two instances running, and says nothing about replacing one of them under load, about partitions, or about a third replica;
