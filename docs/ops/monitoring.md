...
It is the one grant retention adds, and it is *not* covered by the provisioning template's default privileges — those reach tables, deliberately never functions ([decision 37](../decisions.md#37--a-grant-that-reaches-the-tables-that-do-not-exist-yet-default-privileges-a-one-time-repair-and-an-upgrade-that-says-so); the rest of the role recipe is in [install.md](install.md#hardening-the-postgres-role-optional)). Without it, audit retention is refused, `retention_refused_tables` goes to 1, the instance logs the grant above by name, and **every other table is still swept** — one missing grant does not stop sessions from being purged. This is the one deployment step retention adds, and it is deliberately a step rather than a default: relying on Postgres' `PUBLIC` execute default would have meant no action for you and the same capability for every other role on a shared cluster.
