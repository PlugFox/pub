...
| D4 | **The default SQLite deployment has no WAL and no busy timeout**, with a hard-coded 5-connection pool (`db-sqlite/src/lib.rs:64-75`). | Two concurrent writers produce `SQLITE_BUSY` → 500. This is the documented `docker run` minimum. |
| D5 | **`docker run` produces an instance nobody can sign in to.** The image sets only `PUB_SERVER__LISTEN`, so it boots in dev mode with an ephemeral JWT keyring, ephemeral OTP pepper and ephemeral KEK ("enrolled TOTP factors become undecryptable when this process exits"), with the in-memory mailer — and email OTP is the always-available sign-in factor. No `VOLUME`, so `docker rm` destroys the registry. | The documented minimum deployment is not a deployment. |
| D6 | **There is no way to obtain a build.** Decision 18 is untouched: no tags, no release workflow, no published image, no version derivation, no upgrade path. No CI job builds the Dockerfile. | Nobody can install this, and a broken Dockerfile merges green. |
...
- **D24** Connection pools hard-coded at 5 with no timeouts and no config keys — a hard ceiling on request concurrency.
