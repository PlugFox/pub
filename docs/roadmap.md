...
- **D37** User suspension does not revoke or disable the account's CLI tokens (S-27 gap): token auth re-derives authority from org membership, never from user status, and no admin-plane token-revocation or revoke-all endpoint exists — so a compromised **sole-Owner** account cannot be fully neutralized by an instance admin (membership removal is blocked by the last-owner rule, suspension only cuts the web plane). Found by the Phase 1 final-wave adversarial review; [security-runbook.md](ops/security-runbook.md#break-glass) documents the honest workaround.
...
- **D39** Org member grants have **no role ceiling**: `add_member`/`change_role` (`admin/src/orgs.rs`) require `ManageMembers` (Admin+) and step-up but never compare the granted level against the actor's own — an org **Admin can grant or promote to Owner**, reaching the org-lifecycle powers decision 19 reserves for Owners. Found while re-verifying the break-glass docs during the Phase 1 final-wave review. The fix wants a decided rule first (grant strictly below your own level vs. only Owners touch Owner) — a decision-log entry, then code; the invariant "≥1 Owner" already lives in the repository layer either way.
