...
- **Retention** (`lifecycle.rs`, [decision 30](decisions.md#30--retention-one-window-per-table-a-delete-that-stays-bounded-and-a-privilege-that-survives-the-feature), [S-23](security.md#5-audit--abuse)). **The only job in the instance that deletes rows**, and **on by default** — a default install whose tables only grow is not a default. One window per table in days (`0` = keep forever): audit 730, sessions 30, invitations 30, notifications 180, download stats off, plus the queue's own three per-state windows read from `[jobs.queue]`. Every delete is one bounded statement and the pass loops to convergence inside a wall-clock budget, so a released backlog — a lowered window, a restored backup, a forward clock correction — is drained across ticks instead of one write-lock hold; the report distinguishes "drained" from "ran out of time" per table. Predicates are safe by construction rather than by configuration: sessions age from `last_seen_at` (a purged row could no longer authenticate) and invitations from `COALESCE(accepted_at, revoked_at, expires_at)` (a live pending invitation is undeletable at any window). On PostgreSQL the audit prune goes through `pub_audit_prune`, a `SECURITY DEFINER` function the app role may execute while still holding no `DELETE` on `audit_log`, and a refused prune disables that one table loudly instead of failing the pass ([S-22.a](security.md#5-audit--abuse)).
