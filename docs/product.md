...
7. Security baseline: audit log (append-only, with a retention window and a streaming NDJSON export — [decision 30](decisions.md#30--retention-one-window-per-table-a-delete-that-stays-bounded-and-a-privilege-that-survives-the-feature)), rate limits with anti-lockout, anti-enumeration, step-up auth, sign-in domain policy, retract + gated hard delete.
